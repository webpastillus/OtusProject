...
#### ClientType
Таблица с типами клиентов (физ.лицо, ИП, юр.лицо и т.д.) 
...
#### Companies
Таблица с названиями компаний клиентов, если они являются юр.лицом
...
#### Clients
Хранит основную информацию о клиентах компании
...
#### ContactInfoType
Таблица с типами контактной информации клиентов
...
#### ClientContactInfo
Хранит контактную информацию о клиенте
...
#### UserRoles
Таблица с ролью/должностью сотрудников в CRM
...
#### Deals
Хранит информацию по заявокам клиентов
...
#### DealsHistory
Хранит информацию об истории изменения статуса заявки
...
#### InteractionTypes
Таблица с типами взаимодействия с клиентами
...
#### Interactions
Хранит информацию об истории взаимодействия с клиентами
...
#### Products
Таблица с продуктами/услугами и их ценой
...
#### DealProducts
Хранит информацию о продуктах в заказах клиента
...
#### ActiveDealsView
Активные сделки с деталями (клиент, менеджер, статус, сумма, дата, сумма оплаты)

| Поле | Описание |
|------|----------|
| `DealId` | ID сделки |
| `ClientFullName` | ФИО клиента |
| `UserFullName` | ФИО ответственного менеджера (может быть NULL) |
| `StatusName` | Статус сделки |
| `DealAmount` | Общая сумма сделки |
| `CreatedDate` |  Дата создания сделки |
| `DealTotalPaidSum` | Оплаченная сумма сделки |

#### EmployeePerformanceView
Эффективность менеджеров (менеджер, роль, кол-во сделок, сумма сделок, кол-во взаимодействий, кол-во уникальных клиентов)

| Поле | Описание |
|------|----------|
| `UserId` | ID сотрудника |
| `FullUserName` | ФИО сотрудника |
| `Email` | Email сотрудника |
| `RoleName` | Должность/роль |
| `DealsCount` | Количество сделок |
| `TotalDealsSum` |  Общая сумма сделок |
| `InteractionsCount` | Количество взаимодействий с клиентами |
| `UniqueClientsCount` | Количество уникальных клиентов |

#### ClientFinancialView
Финансовый отчёт по клиентам (клиент, кол-во заказов, сумма заказов, сумма оплат, дата последнего заказа)
...
| `ClientCompanyName` | Название компании |
| `TotalDeals` | Количество сделок |
| `TotalDealsAmount` |  Общая сумма всех сделок |
| `TotalPaidSum` | Общая сумма оплат |
...
Оплата выполняется через процедуру [`CALL "RegisterPayment"();`](#RegisterPayment). Проверка на натуральное число происходит в коде CRM (можно вынести в БД). Если внесена полная оплата, то обновляем статус на 4 - Оплачен. Имеется возможность заплатить больше и оставшаяся сумма останется на балансе клиента (данный функционал можно добавить в БД, например поле Balance в [`Client`](#Client) и триггеры/процедуры для его обновления) 
